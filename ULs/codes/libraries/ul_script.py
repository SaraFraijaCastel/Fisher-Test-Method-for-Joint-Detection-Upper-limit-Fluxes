...
def generate_ul_script(GRBINFO, transit, PATH_SH, OUTPUT_folder, RECALCULATED, EBL, PSF, config):
    ...
    OUTPUT_FILE = f"{OUTPUT_folder}{sep}UpperLimit_{transit}_{EBL}.csv"
    ...
    for grb in Data_2.index:  
        ...
        line += (
            ...
            f"--ebl {EBL},{z} --pivot 1 --minE {Emin} --maxE {Emax} -V 3)\n"
        )
...
"""
    ...
    file_path = f"{PATH_SH}UL{transit}_{EBL}_{PSF}_{LABEL_UL_CODE}.sh"
...
def run_ul_scripts(config):
    ...
    PATH_GENERAL=config['PATH_GENERAL']+sep
    os.makedirs(PATH_SH,exist_ok=True)+sep
    ...
    for EBL in EBL_list:
        ...
        script1 = f"{PATH_SH}UL1_{EBL}_{bin_size}_{config['LABEL_UL_CODE']}_paral.sh"
        script2 = f"{PATH_SH}UL2_{EBL}_{bin_size}_{config['LABEL_UL_CODE']}_paral.sh"
